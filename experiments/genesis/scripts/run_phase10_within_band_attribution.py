...
def build_stats_rows(detail_df):
    rows = []
    grouped = detail_df.groupby(["dataset_name", "target_layer", "target_layer_type", "site"])
    for (dataset_name, target_layer, target_layer_type, site), group in grouped:
        ...
        semantic_zero = signflip_test(semantic, seed=1000 + int(target_layer))
        random_zero = signflip_test(random, seed=2000 + int(target_layer))
        semantic_vs_random = paired_signflip_test(semantic, random, seed=3000 + int(target_layer))
        rows.append(
            {
    ...
    for (dataset_name, site), group in semantic_only.groupby(["dataset_name", "site"]):
        pivot = group.pivot(index="item_name", columns="target_layer", values="delta_from_baseline_signed_label_margin").dropna()
        layers = [int(layer) for layer in sorted(pivot.columns)]
        for layer_a, layer_b in itertools.combinations(layers, 2):
            contrast = paired_signflip_test(
                pivot[layer_a].to_numpy(dtype=np.float64),
                pivot[layer_b].to_numpy(dtype=np.float64),
                seed=4000 + 31 * int(layer_a) + int(layer_b),
            )
            rows.append(
                {
                    ...
                    "site": site,
                    "layer_a": int(layer_a),
                    "layer_a_type": layer_type_name(layer_a, FOX_LAYER_INDICES),
                    "layer_b": int(layer_b),
                    "layer_b_type": layer_type_name(layer_b, FOX_LAYER_INDICES),
                    "n_items": int(pivot.shape[0]),
        ...
        fox_cols = [layer for layer in layers if int(layer) in FOX_LAYER_INDICES]
        gla_cols = [layer for layer in layers if int(layer) not in FOX_LAYER_INDICES]
        if fox_cols and gla_cols:
            grouped_means = pd.DataFrame(
                {
                    "fox": pivot[fox_cols].mean(axis=1),
                    "gla": pivot[gla_cols].mean(axis=1),
                }
            ).dropna()
            if not grouped_means.empty:
                contrast = paired_signflip_test(
                    grouped_means["fox"].to_numpy(dtype=np.float64),
                    grouped_means["gla"].to_numpy(dtype=np.float64),
                    seed=5000 + len(fox_cols) + len(gla_cols),
                )
                rows.append(
                    {
                        "comparison_type": "semantic_architecture_contrast",
                        "dataset_name": dataset_name,
                        "site": site,
                        "layer_group_a": "fox",
                        "layer_group_b": "gla",
                        "layer_group_a_members": ",".join(str(layer) for layer in fox_cols),
                        "layer_group_b_members": ",".join(str(layer) for layer in gla_cols),
                        "n_items": int(grouped_means.shape[0]),
                        "mean_a": contrast["mean_a"],
                        "mean_b": contrast["mean_b"],
                        "mean_a_minus_b": contrast["mean"],
                        "ci95_low": contrast["ci95_low"],
...
def save_plot(summary_df, stats_df, output_path):
    ...
    datasets = list(summary_df["dataset_name"].drop_duplicates())
    layers = sorted(summary_df["target_layer"].drop_duplicates())
    fig, axes = plt.subplots(len(datasets), 1, figsize=(10.0, 4.0 * len(datasets)), squeeze=False)
    for row_idx, dataset_name in enumerate(datasets):
        ax = axes[row_idx, 0]
        sub = summary_df[summary_df["dataset_name"] == dataset_name].copy()
        stat_sub = stats_df[(stats_df["comparison_type"] == "semantic_vs_random") & (stats_df["dataset_name"] == dataset_name)].copy()
        x = np.arange(len(layers))
        width = 0.36
        semantic = sub[sub["control"] == "semantic"].set_index("target_layer").reindex(layers)
        random = sub[sub["control"] == "random"].set_index("target_layer").reindex(layers)
        ...
        for idx, layer in enumerate(layers):
            row = stat_sub[stat_sub["target_layer"] == layer]
            if row.empty:
                continue
            pvalue = float(row.iloc[0]["semantic_vs_random_pvalue"])
            y = max(
                semantic.iloc[idx]["delta_from_baseline_mean_signed_label_margin"],
                random.iloc[idx]["delta_from_baseline_mean_signed_label_margin"],
            )
            ax.text(idx, y + 0.002, f"p={pvalue:.3f}", ha="center", va="bottom", fontsize=8)
        tick_labels = [f"L{layer} ({layer_type_name(layer, FOX_LAYER_INDICES)})" for layer in layers]
        ax.axhline(0.0, color="black", linewidth=1.0, alpha=0.6)
        ax.set_title(f"{dataset_name} | attn_output within-band attribution")
        ax.set_xticks(x)
...
def main():
    parser = argparse.ArgumentParser(description="Phase 10S: within-band GLA vs FoX attribution at shared intervention sites")
    ...
    parser.add_argument("--output-csv", type=str, default="logs/phase10/within_band_attribution_summary.csv")
    ...
    parser.add_argument("--target-layers", type=str, default="7,8,9,10,11")
    ...
    for dataset_name, eval_json in zip(dataset_labels, eval_jsons):
        ...
        for item in prepared:
            ...
            metrics.update({"dataset_name": dataset_name, "control": "baseline", "site": "baseline"})
    ...
    summary = (
        conditioned.groupby(
            ["dataset_name", "target_layer", "target_layer_type", "site", "control", "mode", "alpha", "position_fraction", "position_label"],
            as_index=False,
        )
        .agg(
