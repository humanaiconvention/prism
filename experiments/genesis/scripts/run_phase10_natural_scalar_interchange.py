...
from scripts.phase10_site_hooks import TensorSiteDirectionalCoeffInterchangeHook, parse_site_list
...
def evaluate_with_interchange_hook(model, prepared_item, anchor_layer, anchor_direction, layer, site, vector, donor_coeff, alpha, position_fraction):
...
def build_stats_rows(slice_df):
    ...
    for (dataset_name, target_layer, site, slice_name, tail_quantile, threshold), group in grouped:
        ...
        semantic_zero = signflip_test(semantic, seed=1000 + 19 * int(target_layer))
        random_zero = signflip_test(random, seed=2000 + 19 * int(target_layer))
        semantic_vs_random = paired_signflip_test(semantic, random, seed=3000 + 19 * int(target_layer))
        rows.append(
            {
                ...
                "counterfactual_gap_threshold": float(threshold) if np.isfinite(threshold) else np.nan,
...
def opposite_label(label):
    ...
    if label == "creative":
        return "math"
    raise ValueError(f"Unsupported label for paired counterfactual donor: {label!r}")


def main():
    parser = argparse.ArgumentParser(description="Phase 10W: natural semantic-scalar interchange at the within-band corridor")
    ...
    parser.add_argument("--output-csv", type=str, default="logs/phase10/natural_scalar_interchange_summary.csv")
    ...
    print("Loading Genesis model for Phase 10W natural scalar interchange...")
    ...
    print("Running paired opposite-label natural scalar interchange evaluation...")
    detail_rows = []
        ...
        for item_idx, prepared in enumerate(dataset["prepared_items"]):
            ...
            for target_layer in target_layers:
                for site_idx, site in enumerate(sites):
                    ...
                    for control_name, vector, donor_coeff, recipient_coeff in (
                        ("semantic", semantic_vec, semantic_donor_coeff, semantic_recipient_coeff),
                        ("random", random_vec, random_donor_coeff, random_recipient_coeff),
                    ):
                        metrics = evaluate_with_interchange_hook(
                            model,
                            prepared,
                            args.anchor_layer,
                            anchor_direction,
    ...
    summary_df = (
        slice_df.groupby(["dataset_name", "target_layer", "site", "slice_name", "tail_quantile", "counterfactual_gap_threshold", "control"], dropna=False)
        .agg(
