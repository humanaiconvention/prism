...
from scripts.run_phase10_h3_ablation_interaction import OProjHeadColumnAblation
...
CONDITION_H3_ABLATE_ONLY = "h3_ablate_only"
CONDITION_SEMANTIC_STEER_ONLY = "semantic_steer_only"
CONDITION_SEMANTIC_STEER_PLUS_H3_ABLATE = "semantic_steer_plus_h3_ablate"
CONDITION_RANDOM_STEER_ONLY = "random_steer_only"
CONDITION_RANDOM_STEER_PLUS_H3_ABLATE = "random_steer_plus_h3_ablate"
CONDITION_ORDER = [
    CONDITION_BASELINE,
    CONDITION_H3_ABLATE_ONLY,
    CONDITION_SEMANTIC_STEER_ONLY,
    CONDITION_SEMANTIC_STEER_PLUS_H3_ABLATE,
    CONDITION_RANDOM_STEER_ONLY,
    CONDITION_RANDOM_STEER_PLUS_H3_ABLATE,
]
...
REQUIRED_PAIR_INTERACTION_COLUMNS = [
    ...
    "ablation_layer", "ablation_head", "vector_key", "mode", "alpha", "position_label",
    "position_fraction", "n_items_per_pair", "baseline_signed_label_margin",
    "h3_ablate_only_signed_label_margin", "semantic_steer_only_signed_label_margin",
    "semantic_steer_plus_h3_ablate_signed_label_margin", "random_steer_only_signed_label_margin",
    "random_steer_plus_h3_ablate_signed_label_margin", "semantic_interaction_signed_label_margin",
    "random_interaction_signed_label_margin",
]
REQUIRED_STATS_COLUMNS = [
    "dataset_name", "source_layer", "target_layer", "target_layer_type", "site", "ablation_layer",
    "ablation_head", "vector_key", "mode", "alpha", "position_label", "position_fraction",
...
def parse_controls(raw_controls):
    ...
    if set(ordered) != set(ALLOWED_CONTROLS):
        raise ValueError("Phase 12A requires exactly the semantic and random matched controls.")
    return ["semantic", "random"]


def add_condition_row(detail_rows, *, baseline, row, prepared_item, args, dataset_name, position_label, condition, control_name):
    ...
    enriched.update(
        {
            ...
            "site": args.site,
            "ablation_layer": int(args.ablation_layer),
            "ablation_head": int(args.ablation_head),
            "vector_key": args.vector_key,
            "condition": condition,
            "control": control_name,
            "steering_enabled": int("steer" in condition),
            "head_ablation_enabled": int("ablate" in condition),
...
def build_summary_df(detail_df):
    ...
    summary_df = summary_df.sort_values(["dataset_name", "target_layer", "site", "_condition_order"]).drop(columns=["_condition_order"])
    return summary_df
...
def build_pair_interaction_df(detail_df):
    ...
    for group_key, group in pair_condition_df.groupby(group_cols, dropna=False):
        ...
        for metric_col, metric_name in METRIC_SPECS:
            ...
            ablate = float(condition_table.loc[CONDITION_H3_ABLATE_ONLY, metric_col])
            semantic_steer = float(condition_table.loc[CONDITION_SEMANTIC_STEER_ONLY, metric_col])
            semantic_both = float(condition_table.loc[CONDITION_SEMANTIC_STEER_PLUS_H3_ABLATE, metric_col])
            random_steer = float(condition_table.loc[CONDITION_RANDOM_STEER_ONLY, metric_col])
            random_both = float(condition_table.loc[CONDITION_RANDOM_STEER_PLUS_H3_ABLATE, metric_col])
            row[f"baseline_{metric_name}"] = baseline
            row[f"h3_ablate_only_{metric_name}"] = ablate
            row[f"semantic_steer_only_{metric_name}"] = semantic_steer
            row[f"semantic_steer_plus_h3_ablate_{metric_name}"] = semantic_both
            row[f"random_steer_only_{metric_name}"] = random_steer
            row[f"random_steer_plus_h3_ablate_{metric_name}"] = random_both
    ...
    if pair_df.empty:
        return pair_df
    return pair_df.sort_values(["dataset_name", "target_layer", "site", "pair_name"]).reset_index(drop=True)
...
def build_stats_rows(pair_interaction_df, *, n_perm=100000, seed=1234):
    ...
    for group_key, group in pair_interaction_df.groupby(group_cols, dropna=False):
        ...
        for metric_idx, (_, metric_name) in enumerate(METRIC_SPECS):
            ...
            seed_base = int(seed) + (metric_idx + 1) * 1000 + 31 * int(common["target_layer"])
            ...
            rows.append(
                _stats_row(common, metric_name, "semantic_interaction_vs_zero", semantic_zero, n_perm=n_perm, seed=seed_base + 1)
            )
            rows.append(
                _stats_row(common, metric_name, "random_interaction_vs_zero", random_zero, n_perm=n_perm, seed=seed_base + 2)
            )
            rows.append(
                _stats_row(
                    common,
...
def main():
    parser = argparse.ArgumentParser(description="Phase 12A: L15-H3 × L11 attn_output circuit-closure interaction")
    ...
    parser.add_argument("--output-csv", type=str, default="logs/phase12/phase12a_h3_steering_circuit_closure.csv")
    ...
    with OProjHeadColumnAblation(model, args.ablation_layer, args.ablation_head, config.head_dim):
        for prepared_item in tqdm(prepared_items, desc="H3 ablation only", leave=False):
    ...
    print("\n=== PHASE 12A: H3 × L11 CIRCUIT-CLOSURE INTERACTION ===")
    ...
    print(f"Ablation: L{args.ablation_layer}-H{args.ablation_head}")
    ...
    for prepared_item in prepared_items:
        ...
        add_condition_row(
            ...
            dataset_name=args.dataset_name,
            position_label=position_label,
            condition=CONDITION_H3_ABLATE_ONLY,
            control_name="none",
        )

    condition_names = {
        "semantic": (CONDITION_SEMANTIC_STEER_ONLY, CONDITION_SEMANTIC_STEER_PLUS_H3_ABLATE),
        "random": (CONDITION_RANDOM_STEER_ONLY, CONDITION_RANDOM_STEER_PLUS_H3_ABLATE),
    }
    for control_name in controls:
        steer_condition, both_condition = condition_names[control_name]
        ...
        try:
            with OProjHeadColumnAblation(model, args.ablation_layer, args.ablation_head, config.head_dim):
                for prepared_item in tqdm(prepared_items, desc=f"{control_name} steer + H3 ablate", leave=False):
    ...
    print("\n--- PHASE 12A PRIMARY INTERACTION STATS ---")
