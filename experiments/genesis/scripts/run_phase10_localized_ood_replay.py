...
def build_stats_rows(detail_df):
    rows = []
    for (dataset_name, site), group in detail_df.groupby(["dataset_name", "site"]):
        ...
        semantic_zero = signflip_test(semantic, seed=101)
        random_zero = signflip_test(random, seed=102)
        semantic_vs_random = paired_signflip_test(semantic, random, seed=103)
        rows.append(
            {
...
def save_plot(summary_df, stats_df, output_path):
    ...
    for ax, dataset_name in zip(axes[0], datasets):
        ...
        stat_sub = stats_df[stats_df["dataset_name"] == dataset_name].copy()
        sites = list(sub["site"].drop_duplicates())
        ...
        for idx, site in enumerate(sites):
            ...
            if not row.empty:
                ...
                ax.text(idx, max(semantic.iloc[idx]["delta_from_baseline_mean_signed_label_margin"], random.iloc[idx]["delta_from_baseline_mean_signed_label_margin"]) + 0.002, f"p={pvalue:.3f}", ha="center", va="bottom", fontsize=8)
...
def main():
    parser = argparse.ArgumentParser(description="Phase 10G: localized OOD replay at L11 sites")
    ...
    parser.add_argument("--output-csv", type=str, default="logs/phase10/localized_ood_replay_summary.csv")
    ...
    parser.add_argument("--sites", type=str, default="block_input,attn_output,o_proj")
                ...
                try:
                    for item in prepared:
                        ...
                        for column in ["signed_label_margin", "label_target_pairwise_prob", "label_accuracy", "anchor_cosine", "math_minus_creative_logprob"]:
                            metrics[f"delta_from_baseline_{column}"] = float(metrics[column] - baseline_df.loc[item["item"]["name"], column])
    ...
    summary = (
        conditioned.groupby(["dataset_name", "target_layer", "site", "control", "mode", "alpha", "position_fraction", "position_label"], as_index=False)
        .agg(
        ...
        .sort_values(["dataset_name", "site", "control"])
    )
