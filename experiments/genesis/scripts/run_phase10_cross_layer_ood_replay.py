...
def build_stats_rows(detail_df):
    rows = []
    grouped = detail_df.groupby(["dataset_name", "target_layer", "site"])
    ...
    for (dataset_name, site), group in semantic_only.groupby(["dataset_name", "site"]):
        ...
        for a, b in ((11, 7), (11, 15), (7, 15)):
            if pivot.empty or a not in pivot.columns or b not in pivot.columns:
                continue
            contrast = paired_signflip_test(pivot[a].to_numpy(dtype=np.float64), pivot[b].to_numpy(dtype=np.float64), seed=4000 + a + b)
            rows.append(
                {
...
def save_plot(summary_df, output_path):
    ...
    for row_idx, dataset_name in enumerate(datasets):
        for col_idx, layer in enumerate(layers):
            ...
            sub = summary_df[(summary_df["dataset_name"] == dataset_name) & (summary_df["target_layer"] == layer)].copy()
            if sub.empty:
                ax.axis("off")
                continue
...
def main():
    parser = argparse.ArgumentParser(description="Phase 10K: cross-layer localized OOD replay across FoX layers")
    ...
    parser.add_argument("--output-csv", type=str, default="logs/phase10/cross_layer_ood_replay_summary.csv")
